...
| 영역 | P0 | P1 |
|---|---|---|
| 센터 | 공개 디렉터리와 1 km 내 거리순 최대 10개 추천 | ADMIN 승인 파트너와 대시보드 전용 관리 계정 |
...
2. `handover_records`, `return_records`를 추가한다. 기존 `USER_CONFIRMED_HANDOVER`를 대시보드 수락 대기열로 조회할 수 있게 한다.
...
6. P0 인계 확정은 사용자 진술이다. 소유자는 P1 센터 수락 전까지 수정·철회할 수 있고 변경은 감사한다.
7. `DRAFT` 정리와 종료 항목 미디어 30일 정리를 예약 작업으로 수행한다.

완료 조건: 인계 대기 항목은 후보에 없고, Vision 실패·누락 특징이 등록을 막지 않으며, 사진이나 매칭 입력 변경은 재분석과 후보 stale을 일으킨다.
...
3. 센터 선택 후 NAVER 지도 딥링크로 이동하고, 앱의 미완료 인계 안내에서 인계를 확정하는지 확인한다.
...
1. 선택된 센터 담당자는 `USER_CONFIRMED_HANDOVER` 대기열에서 실물 존재를 확인하고 `CENTER_CONFIRMED`로 수락하거나 거절한다.
